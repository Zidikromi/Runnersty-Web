...
const reviews = [
  { name: 'Nadiya Triandini', stars: 5, text: 'Luar biasa! Rute yang menantang tapi sangat teroganisir. Panitia ramah dan semua berjalan lancar dari awal sampai finish.', initials: 'NT' },
  { name: 'Dianta Annika', stars: 5, text: 'Tiada kata yang bisa menggambarkan betapa serunya event ini. Trek alam yang indah, vibes komunitas yang hangat!', initials: 'DA' },
  { name: 'Mario Mikha', stars: 5, text: 'Salah satu race terbaik yang pernah saya ikuti. Pemandangan alam Dago Giri luar biasa dan energy crowd di garis finish.', initials: 'MM' },
  { name: 'Rina Salsabila', stars: 5, text: 'Event yang benar-benar berkesan. Organisasi rapi, rute indah, dan komunitas yang sangat supportif.', initials: 'RS' },
  { name: 'Budi Santoso', stars: 5, text: 'Pengalaman trail running terbaik yang pernah ada. Medannya seru, pemandangan memukau, dan atmosfernya luar biasa.', initials: 'BS' },
];
...
const Stars = ({ count }) => (
  <div className="flex gap-0.5">
    {Array.from({ length: 5 }).map((_, i) => (
      <svg key={i} className={`w-4 h-4 ${i < count ? 'text-red-500' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function Testimonials() {
  return (
    <section className="w-full bg-[#E8E2D0] py-14 overflow-hidden">
      <style>{`
        .testimonials-swiper .swiper-slide { transform: scale(0.88); filter: blur(2px); opacity: 0.6; transition: all 0.4s ease; }
        .testimonials-swiper .swiper-slide-active { transform: scale(1); filter: blur(0px); opacity: 1; }
      `}</style>
            ...
            {reviews.map((r) => (
              <SwiperSlide key={r.name}>
                <motion.div
                  ...
                  className="bg-white rounded-2xl p-5 flex flex-col gap-3 shadow-sm h-[240px] md:h-[220px]"
                >
                  <Stars count={r.stars} />
