...
export default function Navbar({ isLanding, showFinalNav, navigateTo, currentPage }) {
  ...
  // Mengunci scroll total pada body dan html saat sidebar terbuka (Mengatasi scroll bocor di iOS/Android)
  useEffect(() => {
    if (isOpen) {
    ...
    <>
      ...
      {!isPrivacyPage && isOpen && (
        <div className="fixed inset-0 z-[99] md:hidden h-[100dvh] w-full overflow-hidden">
          ...
          {/* Panel */}
          {/* Menggunakan h-[100dvh] agar tinggi pas dengan dynamic viewport di browser mobile */}
          <div className="absolute top-0 right-0 h-[100dvh] w-[80%] max-w-xs bg-[#2D3A1F] flex flex-col justify-between p-8 shadow-2xl">
